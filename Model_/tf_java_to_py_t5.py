# -*- coding: utf-8 -*-
"""tf_java_to_py_T5.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1QC3AvbKUGXhWBBE5yv4ppXQTU2NL-wGA
"""
...
file_paths = [
    "/content/drive/MyDrive/Hackthon/train.jsonl",
]

# List to store dataframes
dfs = []

# Read and create dataframe for each JSONL file
...
data_train = pd.concat(dfs, ignore_index=True)
data_train.head(3)
data = data_train
data['java'] = data['java'].apply(lambda x: '\n'.join(x))
data['python'] = data['python'].apply(lambda x: '\n'.join(x))
train_data = data
train_data = train_data.drop(['id'],axis=1)
train_data.head(2)
train_data.to_csv("/content/drive/MyDrive/Hackthon/train.csv")
...
def run(args):
    ...
    #dataset_java = load_dataset("codeparrot/xlcost-text-to-code", "Java-program-level")
    dataset_java_train = pd.read_csv("/content/drive/MyDrive/Hackthon/train.csv")
    # dataset_java_valid = pd.read_csv("/content/drive/MyDrive/Hackthon/java_validation.csv")
    # dataset_java_test = pd.read_csv("/content/drive/MyDrive/Hackthon/java_test.csv")

    dataset_java_train_sampled = dataset_java_train.sample(frac=0.4, random_state=42)
    # dataset_java_valid_sampled = dataset_java_valid.sample(frac=0.5, random_state=42)
    # dataset_java_test_sampled = dataset_java_test.sample(frac=0.5, random_state=42)
    dataset_java_train_sampled.to_csv("train_java.csv")

    logger.info(" Loading Data Files")

    # dataset_split = DatasetDict({
    # 'train': dataset_java_train_sampled,
    # 'validation': dataset_java_valid_sampled,
    # 'test': dataset_java_test_sampled
    #   })

    dataset = load_dataset('csv', data_files="/content/train_java.csv")
    # train test split
    dataset = dataset['train'].train_test_split(0.2, shuffle=False)
...
dest_dir = '/content/drive/MyDrive/Hackthon/'
...
def run_predict(args, text):
    ...
    generated_code = model.generate(
        ...
        max_length=args.max_target_length, top_p=0.95, top_k=50, repetition_penalty=1, num_return_sequences=1
    )
...
def predict_from_dataset(args):
    ...
    text = test_dataset[index]['java']
    code = test_dataset[index]['python']
